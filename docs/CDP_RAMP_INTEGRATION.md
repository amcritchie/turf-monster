...
- **`Cdp::OfframpPollJob`** — `GET /onramp/v1/sell/user/{partner_user_ref}/transactions?page_size=50` (page_size **defaults to 1** — always pass it) [^10][^13]. Per docs: "avoid polling immediately after generating the URL; wait until the send transaction is created… using exponential backoff" [^11] — start on return-page hit: 10s → 30s → 1m → 2m → 5m, stop at terminal status or deadline + grace. Status enum (API reference): `TRANSACTION_STATUS_CREATED | EXPIRED | STARTED | SUCCESS | FAILED`; the guide page lists only STARTED/SUCCESS/FAILED — handle unknown values defensively [^13]. Response fields used: `to_address`, `sell_amount`, `from_address`, `tx_hash`, `coinbase_fee` (null unless offramping to fiat). `transactions[]` is reverse chronological; with our per-session `partner_user_ref` the first row is ours [^13].
