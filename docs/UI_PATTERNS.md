...
Selection badges are fixed-width (`w-28`), sorted by game kickoff time, showing turf score (e.g., `x3`) before game completes and points (goals x turf score) after. Badges float right with score rightmost (`min-width: 4.5rem`). Non-integer values show decimal portion in smaller font. A compact payout ladder appears above the leaderboard with `Projected` copy before settlement and `Settled` copy after grading. Row payout badges say `Projected $X` before settlement and `Won $X` after settlement; tie copy explains that tied entries split the prize slots they span. After settling — paid rows get primary ring, unpaid rows dimmed, and rank column shows actual rank (from `entry.rank`).
