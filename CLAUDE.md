...
- **Deploy**: `bin/deploy [app]` — multi-app aware (`turf-monster`→`heroku` = devnet prod; `turf-monster-mainnet`→`heroku-mainnet`, with a remote↔app guard so code can't ship to the wrong app). Pre-flight (clean tree, network-keyed IDL pin, Stripe live-key on both prod apps, tests) → push → smoke-check (rolls back on failure). Migrations run in Heroku's release phase, not the script. An IDL-hash bump is handled automatically via the `EXPECTED_IDL_HASH` **allow-list** — `bin/deploy` widens it to `{old,new}`, pushes, then tightens to `{new}`, so both slugs verify across the release with no unverified window (no manual `BYPASS_IDL_CHECK` dance; BYPASS remains as break-glass). Mainnet requires confirmation (typed app name, or `--yes` non-interactive). `bin/deploy --help` for options. **One-time mainnet first deploy**: follow `MAINNET_LAUNCH.md` instead.
