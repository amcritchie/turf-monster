...
- `/cdp/onramp_sessions` + `/cdp/offramp_sessions` (POST, JSON `{ url: }`), `/cdp/onramp/return` + `/cdp/offramp/return` (GET, Coinbase redirectUrl targets — UX signal only), `/cdp/ramp_status/:partner_user_ref` (GET JSON, local row poll) — Coinbase CDP buy/cash-out USDC. All 404 unless `ENABLE_CDP_RAMP=true`; session POSTs are auth + geo + catalog gated and throttled per-user (`cdp_sessions/user`). See `docs/CDP_RAMP_INTEGRATION.md` §8.
