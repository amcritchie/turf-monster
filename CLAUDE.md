...
- Prefer the ecosystem launcher from McRitchie Studio: `cd ../mcritchie-studio && bin/agent-worktree new turf-monster <slug>`. It creates `turf-monster/.worktrees/<slug>` on `feat/<slug>`, copies `.env`, writes isolated stack env, and prints the stack commands.
- `bin/worktree new <slug>` remains as a Turf-local compatibility wrapper and also creates `.worktrees/<slug>`.
...
**Port `:3100` is the canonical/main dev server — reserve it for the primary.** External callbacks are wired to it: the Stripe webhook forward (`localhost:3100/webhooks/stripe`), the Google OAuth redirect URIs, and the dev mailer host (`development.rb` `default_url_options` falls back to `3100`). Worktree stacks run on **3101+** and are fine for general UI/backend dev, but any flow with an external callback — **Stripe, Google OAuth, MoonPay, webhooks, emailed magic-links** — must be exercised on the primary `:3100`.

**Running a worktree's live dev stack:** prefer `cd ../mcritchie-studio && bin/agent-worktree up turf-monster <slug>`. It starts web + Sidekiq on the allocated port with isolated Redis, database, and session cookie settings, then prints the URL. `bin/parallel-server [PORT] [REDIS_DB]` remains for a quick unnamed running stack and now defaults to `.worktrees/parallel-<PORT>`.

Why the isolation matters (each of these silently collides otherwise):
- **Port + mailer links** — the dev mailer host port reads `APP_PORT` (default 3100) in `config/environments/development.rb`, so emailed links (e.g. magic links) point at *this* server instead of `:3100`.
...
- **Heroku app**: `turf-monster-mainnet` (the only public prod app). The second devnet-prod deploy target — the `turf-monster` app on `turf.mcritchie.studio` — was decommissioned 2026-06. The devnet **cluster** is unaffected (local dev, CI devnet-nightly, and `Solana::Config`'s network selection all still use it); only the redundant second *prod deploy* is gone.
- **URL**: https://app.turfmonster.media
- **Database**: Heroku Postgres
- **Redis**: Heroku Redis — `REDIS_URL` set automatically
- **Deploy**: `bin/deploy` — single mainnet target (`turf-monster-mainnet` → `heroku-mainnet` remote; no args, no app argument). Pre-flight (clean tree, network-keyed IDL pin, Stripe live-key, tests) → push → smoke-check (rolls back on failure). Migrations run in Heroku's release phase, not the script. An IDL-hash bump is handled automatically via the `EXPECTED_IDL_HASH` **allow-list** — `bin/deploy` widens it to `{old,new}`, pushes, then tightens to `{new}`, so both slugs verify across the release with no unverified window (no manual `BYPASS_IDL_CHECK` dance; BYPASS remains as break-glass). Because it ships real funds, the deploy requires confirmation (typed app name, or `--yes` non-interactive). `bin/deploy --help` for options. **One-time mainnet first deploy**: follow `MAINNET_LAUNCH.md` instead.
...
- **Env vars** (currently set on Heroku): `RAILS_MASTER_KEY`, `SECRET_KEY_BASE`, `DATABASE_URL` (auto), `REDIS_URL` (auto), `RAILS_SERVE_STATIC_FILES`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `RESEND_API_KEY`, `MAILER_FROM`, `MANAGED_WALLET_ENCRYPTION_KEY`, `EXPECTED_IDL_HASH`, `SOLANA_PROGRAM_ID`. Flag vars: `ENABLE_AGE_ATTESTATION` (legal-age attestation checkbox on all signup surfaces — **unset = off, the current prod state**; operator will re-enable after the first contest; see `AppFlags.age_attestation?`), `ENABLE_CDP_RAMP`, `ENABLE_TEST_SCAFFOLDING`, `ENABLE_WEB2_USDC_ENTRY` (**DEFAULT ON — kill-switch**, unlike the others; web2/managed users fund entries with USDC token-first→USDC via server-signed `enter_contest`; set `false` to revert web2 to token-only; web3 unaffected; see `AppFlags.web2_usdc_entry?`). See `.env.example` for the full documented set (including `SOLANA_ADMIN_KEY` / `SOLANA_RPC_URL` for local dev).
...
- **SolanaStudio gem** — `gem "solana-studio", "~> 0.4.7"` (RubyGems; current 0.4.7, locked). Pure-Ruby primitives — Solana::Client (RPC), Solana::Borsh, Solana::Transaction, Solana::SplToken, Solana::Keypair. 0.4.3 fixed the `Net::HTTP::Post.new(@uri.path)` query-string-dropping bug that broke Helius auth in 0.4.2 (see solana-studio PR #1 + CHANGELOG).
...
- `solana_utils` — shared Solana/crypto utilities: `lockedFetch`, `refreshBalance` (hits `/admin/usdc_balance`), `refreshBalanceDelayed`, **`refreshSession`** (hits `/account/session_refresh` — THE single page-load hydrate; the layout's `hydrateNavbar()` calls it on every load, and it's also the post-on-chain-success call: balance pill + 🎟️ badge + seeds bar + `$store.session` cents + wallet tiles in one shot), `updateNavTokens`, `CONFETTI_COLORS`. Both hydrate paths paint the navbar pill as **USDC + USDT combined** and call the shared `updateWalletTiles(data)` fanout — any page can subscribe a balance readout by tagging an element `data-wallet-tile="usdc|usdt|sol|tokens"` (how /account's per-currency Identities tiles stay live; `walletRefresh` in `shared/_alpine_factories` powers the Refresh Wallet button there). Attached to `window` for backward compatibility.
- `solana_errors` — Solana error message parser (`parseSolanaError`). Friendly mappings for the username codes 6020/6021/6022 + 6027 `EntryFeeNotSet`; server-side twin is `app/services/solana/error_interpreter.rb`.
- `solana_stores` — Alpine.js wallet watcher store (detects Phantom `accountChanged` on web3 sessions, silent re-auth). Re-auth runs **only in the visible tab** (the server session has a SINGLE nonce slot — see Known Gotchas), with an in-flight guard, one automatic retry on rejection, then a loud fallback to the Connect Wallet modal. A Phantom 4001 (user dismissed the prompt) is respected — no retry, no fallback.
...
- `state_fanout` — `window.StateFanout.apply(stateType, payload, opts)`. See "State fanout pattern" below for the contract. Registered handlers: `seeds`, `cdp_ramp`. Add a new handler when the next on-chain state delta needs to fan out to localStorage + a navbar event.
...
| `cdp_ramp` | `/cdp/ramp_status/:ref` terminal statuses (applied by the cdp-ramp modal's poll loop) | none — the USDC pill is server-cache-backed; the handler calls `refreshBalance()` now + again after the 60s server cache TTL when funds moved | `cdp-ramp-update` |
...
**From the engine:** `Studio::ErrorHandling`, `ErrorLog` model, `Sluggable` concern, auth controllers, error log views, theme system, `_theme_toggle_morph` partial (spinner/toggle swap), `showNavSpinner`/`hideNavSpinner` globals, **`Studio::S3`** + **`Studio::ImageCache`** + `ImageCache` model, and the **image-upload UI** (studio-engine v0.4.12): `studio/_cropper_assets` (cropper.js + the `imageUploadHost` / `submitFormWithProgress` / `cropPhotoModal` factories) + the `studio/modals/_crop_photo` and `_saving` partials. Add a cropped image upload with `x-data="imageUploadHost({ aspectRatio:, maxWidth:, filename:, saving:, … })"` + `render "studio/cropper_assets"` on the page, and register crop-photo/saving in the modal-host block. NB: the modal host (`studio/modals/_host`) is **overridden locally** (`app/views/studio/modals/_host.html.erb`) for the `swap()`/`advance()` animations — engine host changes don't reach this app, which is why the image-upload JS ships page-scoped via `cropper_assets`, not the host.
...
- **Balance system (v0.16)**: USDC and USDT live in each user's own ATA — no custodial vault balance. Web2/managed-wallet users have their keypair server-held (`encrypted_web2_solana_private_key`), but the funds destination is still their ATA. The navbar pill shows **USDC + USDT combined** (operator request 2026-06-10): `display_balance` is cache-first + non-blocking — it sums the cached `usdc_cache_key` + `usdt_cache_key` values (`combined_balance`), returns `nil` when both are cold ("loading"), and NEVER issues an RPC on the render path; the client-side hydrate (`refreshSession()` on page load, `refreshBalance()` elsewhere) fills the pill and warms the caches via `fetch_navbar_hydrate`. The `/admin/usdc_balance` endpoint's `balance` field is the same combined sum. The **/account tiles stay per-currency** (`data-wallet-tile`). `User#entry_token_balance` reads on-chain `EntryTokenAccount` PDAs. Balance + token reads cached 60s in Redis; callers that mint/consume tokens MUST follow up with `user.bust_entry_tokens_cache!` to propagate to the navbar + eligibility blocker in the same request cycle.
...
- **User** — name, username, email (nullable), solana_address, wallet_type, role, slug. Balance is on-chain USDC/USDT. **Reserved usernames**: `User::RESERVED_USERNAME_PREFIXES` mirrors turf-vault's on-chain prefix list (`programs/turf_vault/src/instructions/set_username.rs`, v0.15.1 audit C2) — validated only on username *change* (grandfathered rows save fine), admins exempt; `ensure_username` filters reserved draws from the generator. `User.turf` keys on `User::TURF_HOUSE_EMAIL` (`turf@mcritchie.studio`), with a `username: "turf"` fallback for legacy rows. Kickoff usernames are claimed per admin wallet via `bin/rails admin:claim_usernames` (`DRY_RUN=1` to preview; DB-only — on-chain `set_username` still owed after). See `docs/AUTH.md`.
- **Contest** — name, tagline, entry_fee_cents, status (`pending`/`open`/`settled` — no `locked`), max_entries, rank, season_id (OPSEC-023), **`accepts_usdt`** (boolean, default false — stamped `true` on every on-chain creation path, which funds the on-chain `entry_fee_by_currency` slot 1 at create; that array is IMMUTABLE after `create_contest`, so contests created before 2026-06-11 have a zero USDT fee on-chain forever and stay USDC-only), slate association, onchain fields, slug. On-chain `lock_timestamp`/`conclusion_timestamp` mirrored to the `starts_at`/`concludes_at` columns. `belongs_to :user` (creator, optional). `has_one_attached :contest_image` (Active Storage). Helpers: `lock_time_display`, `active_entry_count`, `locks_at` (alias for `starts_at`), `locked?`, `concluded?`, `live?`, `games_by_phase`. Contest's matchups come from `SlateMatchup` via the associated `Slate` — there is no separate `ContestMatchup` model.
...
- **Current** — `ActiveSupport::CurrentAttributes`. Attributes: `user`, `outbound_source`, `true_admin` (OPSEC-046: real admin behind an impersonated session), plus `vault_state` / `vault_state_fetched` / `vault_state_error` (per-request memo for the admin navbar's on-chain vault badges). Request- and Sidekiq-job-scoped (auto-resets). Set by `ApplicationController#set_current_context` and jobs/services to flow `user` + polymorphic domain object through the logging layers (especially `OutboundRequest`). No slug.
...
- **OutboundRequest** — `service` (string: `stripe` / `solana_rpc` / `moonpay` / `cdp`), `status_code`, `error_class`, `created_at` (manual, no `updated_at`). `belongs_to :source` (polymorphic, optional), `belongs_to :user` (optional), `belongs_to :acting_admin` (`class_name: "User"`, optional — OPSEC-046, stamped from `Current.true_admin` so a call made while impersonating attributes back to the admin). Scopes: `recent`, `for_service`, `failed`, `successful`. Predicates: `failed?`, `successful?`. Immutable audit log of every Stripe + Solana RPC + Coinbase CDP call (set via `Stripe::Instrumentation` + prepended `Solana::ClientLogger` + `Cdp::Client` directly); retention sweeper trims 90d ok / 180d failed. No slug.
- **ImpersonationLog** — `admin_id` / `target_user_id` (both `belongs_to :…, class_name: "User"`), `action` (enum `enter:0` / `exit:1`), `ip`, `user_agent`, `reason` (nullable), `created_at` (manual, no `updated_at`). Scope: `recent`. Immutable audit trail of admin "act as user" impersonation (OPSEC-046) — one row on enter, one on exit (incl. `reason: "logout"` when an impersonating admin logs out). Written by `Admin::ImpersonationsController` + `SessionsController#destroy`. No slug. See the impersonation seam in `ApplicationController` (`true_user` / `current_user` override / `impersonating?` / `verify_session_token` binding to `true_user`).
...
- **CdpRampTransaction** — one row per Coinbase CDP hosted-widget session (onramp = buy USDC, offramp = cash out). `belongs_to :user`. Enums: `direction` (onramp/offramp), `status` (local lifecycle `initiated → token_minted → returned → cdp_created → sending → sent → success | failed | expired | abandoned`), `wallet_mode` (web2 managed / web3 Phantom, `prefix: :wallet`). `partner_user_ref` (`"tm-<user_id>-<id>"`, unique, < 50 chars, assigned `after_create`), `cdp_status` (raw CDP string, verbatim), `coinbase_transaction_id` (unique — poll/webhook upsert idempotency key), `to_address`, `sell_amount_value:decimal` (read via `#sell_amount` → BigDecimal), `raw_payload:jsonb`, `cashout_deadline_at`, `sent_signature`, `confirmed_at` (the fresh explicit user confirmation gating the offramp send), `broadcast_at` (stamped by `mark_sending!` — the broadcast-attempt time anchoring the send job's blockhash-lapse verdict; NEVER anchor that on `confirmed_at`). State transitions go through guarded `mark_token_minted!` / `mark_returned!` / `mark_cdp_created!` / `mark_sending!(sig)` / `mark_sent!` / `mark_success!` / `mark_failed!` / `mark_expired!` (idempotent, never rewind; no raw `status=` writes) — plus `reset_failed_send!`, the ONE deliberate rewind (sending → cdp_created), allowed only after on-chain verification proved a broadcast definitively failed. Scopes: `recent`, `active` (non-terminal); predicates `terminal?` / `pre_cdp?`. Services in `app/services/cdp/` (Auth/Client/SessionTokenService/OnrampUrl/OfframpUrl/Catalog/**OfframpDestination** — on-chain resolver for the §10 to_address owner-vs-token-account ambiguity; gates EVERY offramp send, fails closed with a typed `ResolutionError`); controllers `Cdp::RampSessionsController` (token mint → `{ url: }`) + `Cdp::ReturnsController` (return pages + `/cdp/ramp_status` local poll) + `Cdp::OfframpSendsController` (§10 send path: managed `confirm_send` → `Cdp::OfframpSendJob`, Phantom `prepare_send`/`sent` — the JS half is `app/javascript/cdp_offramp_send.js` `window.buildAndSendOfframpUsdcTransfer`); Sidekiq pollers `Cdp::OnrampPollJob`/`Cdp::OfframpPollJob` (started at session-mint time via `schedule_from_mint` (+60s) — never hinged on the return redirect — and (re-)scheduled on return hits; 10s→30s→1m→2m→5m self-re-enqueue, stop at terminal or deadline+grace, and at deadline+grace only rows with NO `coinbase_transaction_id` are expired — a buy CDP knows about is left for the phase-2 sweep; offramp poll also does the §10 to_address/deadline discovery). `Cdp::OfframpSendJob` is the managed-wallet server-signed USDC send: guards (fresh `confirmed_at` ≤10min, status `cdp_created`, deadline−3min, destination resolution, ATA balance) then persists the tx signature (status → `sending`) BEFORE broadcasting (built via `Solana::Vault#build_user_usdc_transfer`, admin fee-payer + user-keypair authority + priority fee), and VERIFIES any persisted signature on-chain before ever considering a re-send (never blind-resend). Flag `AppFlags.cdp_ramp?` (ENABLE_CDP_RAMP — Cdp::BaseController 404s everything when off). **Frontend**: `modals/_cdp_ramp.html.erb` (one modal id `cdp-ramp`, step machine on props — preflight/opening/waiting/awaiting-cdp/send/settling/done/failed/error) driven by the `cdpRampFlow()` factory in `shared/_alpine_factories.html.erb`; entry points = `/wallet` Buy USDC + Cash out buttons (flag+geo gated, geo-blocked renders disabled+explainer, cash-out client-gates on `$store.session.usdcCents`) and a Buy-USDC option in `modals/_wallet_deposit`; the `/cdp/*/return` pages deep-link back into the modal via a `#cdp-return-config` JSON block; terminal statuses fan out through `StateFanout('cdp_ramp')`. Coinbase always opens via `window.open` new tab — NEVER iframe. Playwright e2e for the live flow is a tracked follow-up. See `docs/CDP_RAMP_INTEGRATION.md`. No slug COLUMN — `partner_user_ref` is the correlation key (`#slug` reads it for ErrorLog target context).
...
- **SiteSetting** — `Sluggable` with hardcoded `name_slug = "site-setting"`. Singleton via `SiteSetting.instance`. `default_og_title` / `default_og_description` strings + `has_one_attached :default_og_image`. Site-wide link-preview (og:image) defaults; admin-edited on `/admin/dashboard`. Resolution + layout wiring live in `OgHelper` (`og_image_url`/`og_title`/`og_description`/`og_image_default?`): page-level `content_for` override → SiteSetting default → static `/og.png`. `LandingPage` also `has_one_attached :og_image` for a per-funnel override. Both attach via the **public-read** S3 service picked per-env in `OgImageAttachable::PUBLIC_OG_SERVICE` (test→`:test`, prod→`:amazon_public`, dev→`:amazon_public_dev`/`:local`) so `attachment.url` is a PERMANENT absolute URL — unfurlers cache it, so a signed/expiring URL from the private `amazon` service would break the preview. See `config/storage.yml`.
...
- AccountsController: update, link_solana, unlink_google, set_inviter, update_username, confirm_username → `target: current_user` (`change_password` is gone — passwords were removed 2026-06-01)
...
### Account & Auth
- `/account` — profile, Google link/unlink, username edit, per-currency wallet tiles + Refresh Wallet. See `docs/AUTH.md`.
...
- `/cdp/onramp_sessions` + `/cdp/offramp_sessions` (POST, JSON `{ url: }`), `/cdp/onramp/return` + `/cdp/offramp/return` (GET, Coinbase redirectUrl targets — UX signal only), `/cdp/ramp_status/:partner_user_ref` (GET JSON, local row poll), `/cdp/offramp/confirm_send` + `/cdp/offramp/prepare_send` + `/cdp/offramp/sent` (POST JSON, §10 offramp send: managed confirm-then-server-send / Phantom prepare-sign-report; keyed on `partner_user_ref`, scoped to the viewer's own offramp rows) — Coinbase CDP buy/cash-out USDC. All 404 unless `ENABLE_CDP_RAMP=true`; session POSTs are auth + geo + catalog gated and throttled per-user (`cdp_sessions/user`). See `docs/CDP_RAMP_INTEGRATION.md` §8/§10.
...
### Rails Tests
- `bin/rails test` — **~1209 runs / ~5595 assertions** (minitest + fixtures; count from PR #130's isolated full-suite run, 2026-06-11), run parallel via `parallelize(workers: :number_of_processors)`; fast wall-clock locally. `COVERAGE=1` (or CI) starts SimpleCov — **~50% line / ~55% branch**, merged across all parallel workers (each worker gets a unique command_name via `parallelize_setup`; before that fix the report collapsed to one "Worker 0" at ~2%). `ENFORCE_COVERAGE=1` gates at 48% line (opt-in, not wired into CI).
...
- **`npm run test:parallel` / `bin/e2e-parallel [N]`** — run the suite ~N× faster **locally** across N isolated test-env stacks (default 4). Each stack = its own Rails server on port `3101+i` + its own Postgres DB (`turf_monster_e2e_pw_<i>`), seeded; the suite is sharded `--shard=i/N` across them, then servers are killed + DBs dropped on exit. Stays off `:3100` and the shared `turf_monster_test` DB, so it never collides with a dev stack or `bin/rails test`. Measured ~2× wall-clock at N=4 (151s → 72s). Why a launcher and not `workers: N`: `/test/reseed` is stop-the-world, so the only safe isolation unit is a whole stack — but in TEST env the *only* shared state is Postgres (cache `:null_store`, jobs `:test`, cable `:test`/async, rack-attack disabled), so a per-stack DB is all that's needed. `bin/e2e-parallel 2 -- theme.spec.js` for a quick check; `-- --retries=1` to absorb the known `geo.spec.js:54` flake.
- `npm run test:headed` / `npm run test:ui` — visual modes
- Config: `playwright.config.js` — Chromium only, port 3100
...
- **The session has a SINGLE nonce slot (delete-before-verify, OPSEC-018)** — every open tab receives Phantom's `accountChanged`, but concurrent re-auths from multiple tabs overwrite each other's nonce, so the prompt the user actually signs verifies against a dead nonce and 401s. Any automatic wallet re-auth MUST run in the visible tab only (`document.visibilityState`), with an in-flight guard; hidden tabs catch up via the layout's `visibilitychange` rehydrate. See `app/javascript/solana_stores.js`.
