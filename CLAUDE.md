...
**The standard — a worktree owns an isolated *bundle*, not just a branch.** Each worktree should own its own **port, Redis DB, Ruby, web server, Sidekiq worker, Postgres dev DB, and session cookie** — allocated on spin-up, given back on `bin/worktree done`. Spin them up together, tear them down together; never leave scratch stacks/DBs running afterward (the operator dislikes sprawl). This principle is host-agnostic and is meant to survive a move to a different machine — only the *concrete values* below (the `ruby@3.3` path, the port/db numbers) are specific to the current host. The mechanics are partly manual today; the one-command target and every known failure mode are specced in the task brief at [`~/projects/dev-stack-smoothing.md`](../dev-stack-smoothing.md).
...
- **Ruby version drift** — `.envrc` (which `PATH_add`s brew `ruby@3.3`) is gitignored, so `bin/worktree new` does NOT copy it → a fresh worktree boots the system Ruby (3.1.x on the current host) while the Gemfile pins 3.3.x, and every `bin/rails` / `bin/tm` fails until fixed. On the current host, prepend the pinned Ruby for the session: `export PATH="/opt/homebrew/opt/ruby@3.3/bin:$PATH"` (or `cp ../turf-monster/.envrc . && direnv allow` for an interactive shell — but direnv does NOT help non-interactive agent shells, so the PATH export is the reliable fix).
- **Redis / Sidekiq workers — ⚠️ DANGER, silent data corruption** — Sidekiq + `Rails.cache` both read `REDIS_URL` (defaults db 0 / db 1). The Postgres DBs are isolated per stack, but the **Sidekiq queue is NOT unless each stack gets its own UNIQUE Redis db** — and both `bin/tm` (`TM_REDIS_DB`) and `bin/parallel-server` gravitate to the *same* default (db 9). When two stacks share a db, stack B's Sidekiq picks up stack A's enqueued job and runs it against **B's** database (where `user_id=6` is a different record) — it "succeeds" with no error and silently does the wrong thing (this is exactly how a new user's on-chain `UserAccount` once never got created). ALWAYS give each stack a distinct db, and verify exactly one Sidekiq is registered before trusting any job: `redis-cli -n <db> smembers processes` (more than one entry = collision). This is the most dangerous worktree footgun — treat a shared Sidekiq db as a stop-the-line bug.
...
- **OutboundRequest** — `service` (string: `stripe` / `solana_rpc` / `moonpay`), `status_code`, `error_class`, `created_at` (manual, no `updated_at`). `belongs_to :source` (polymorphic, optional), `belongs_to :user` (optional), `belongs_to :acting_admin` (`class_name: "User"`, optional — OPSEC-046, stamped from `Current.true_admin` so a call made while impersonating attributes back to the admin). Scopes: `recent`, `for_service`, `failed`, `successful`. Predicates: `failed?`, `successful?`. Immutable audit log of every Stripe + Solana RPC call (set via `Stripe::Instrumentation` + prepended `Solana::ClientLogger`); retention sweeper trims 90d ok / 180d failed. No slug.
