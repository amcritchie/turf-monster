...
class WalletTopupTest < ActionDispatch::IntegrationTest
  ...
  test "the wallet-topup modal reads the USDC balance from the session payload" do
    get contests_path
    assert_response :success
    # Balance line is sourced from $store.session.usdcCents (cents -> dollars),
    # with a graceful em-dash when the field is null (cold cache).
    assert_includes response.body, "$store.session.usdcCents"
    assert_includes response.body, "—"
  end

  test "the primary CTA hands off to the cdp-ramp buy preflight" do
    get contests_path
    assert_response :success
    assert_includes response.body, %(data-topup-rail="coinbase")
    assert_includes response.body,
