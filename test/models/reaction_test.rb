...
class ReactionTest < ActiveSupport::TestCase
  ...
  test "valid with an allowed emoji" do
    assert Reaction.new(message: @message, user: @user, emoji: "❤️").valid?
  ...
  test "a user can only use a given emoji once per message" do
    Reaction.create!(message: @message, user: @user, emoji: "❤️")
    dup = Reaction.new(message: @message, user: @user, emoji: "❤️")
  ...
  test "different users can use the same emoji on one message" do
    Reaction.create!(message: @message, user: @user, emoji: "❤️")
    assert Reaction.new(message: @message, user: users(:alex), emoji: "❤️").valid?
  end
