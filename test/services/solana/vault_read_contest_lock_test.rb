...
# v0.17 added `lock_timestamp: i64` to the on-chain Contest (carved out of
# _reserved, after `bump`). read_contest decodes byte-by-byte with hardcoded
# offsets, so this round-trips a hand-built Contest buffer to guard against
# offset drift — the named risk when the account layout bumps.
class Solana::VaultReadContestLockTest < ActiveSupport::TestCase
  # Build a Borsh Contest account buffer matching the v0.17 field order.
  def build_contest_buffer(lock_timestamp:, status: 0)
    ...
    reserved   = "\x00".b * 24                                 # [u8;24]
    disc + contest_id + admin + creator + season_id + prize_pool +
      fee_by_cur + fees + max_e + cur_e + st + payouts + bump + lock + reserved
  end
